...
if [ "$?" -eq "0" ]; then
  ...
  echo ""
  curl -X PUT -H 'Content-Type: application/x-tar' -H 'Content-Encoding: gzip' --data-binary "@$doc.tar.gz" --user "$username:$password" "https://hackage.haskell.org/package/$1-$2/docs"
  exit $?
else
